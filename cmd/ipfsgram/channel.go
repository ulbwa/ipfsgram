...
// access), list, and removal backed by internal/remove's plan/execute split.
...
func newChannelAddCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runChannelAdd(cmd, args[0])
		},
	}
}

// runChannelAdd probes every bot's access to the channel, stores it and prints
// the resulting membership matrix.
func runChannelAdd(cmd *cobra.Command, arg string) error {
	tgID, err := strconv.ParseInt(arg, 10, 64)
	if err != nil {
		return fmt.Errorf("invalid tg_id %q", arg)
	}
	ctx := cmd.Context()
	...
	infos, title, members := a.probeChannelAccess(ctx, bots, tgID)
	if members == 0 {
...
// probeChannelAccess probes each bot's access to the channel, returning the
// per-bot info, the channel title (from the first member bot) and the member
// count.
func (a *app) probeChannelAccess(ctx context.Context, bots []store.Bot, tgID int64) (map[int64]telegram.ChannelInfo, string, int) {
...
func newChannelRemoveCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runChannelRemove(cmd, args[0], yes)
		},
	}
...
// runChannelRemove confirms the impact and removes the channel's records.
func runChannelRemove(cmd *cobra.Command, arg string, yes bool) error {
	tgID, err := strconv.ParseInt(arg, 10, 64)
	if err != nil {
		return fmt.Errorf("invalid tg_id %q", arg)
	}
