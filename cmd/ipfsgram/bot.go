...
// probing), list, and the two-step removal flow backed by internal/remove.
...
func newBotAddCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runBotAdd(cmd, args[0])
		},
	}
}

// runBotAdd validates the token, stores the bot and probes its membership of
// every channel.
func runBotAdd(cmd *cobra.Command, token string) error {
	ctx := cmd.Context()
	...
	member, err := a.probeBotMembership(ctx, botID, token, channels)
	if err != nil {
		return err
	}
...
// probeBotMembership probes the bot's access to each channel, upserts the
// membership rows and returns how many channels it is a member of.
func (a *app) probeBotMembership(ctx context.Context, botID int64, token string, channels []store.Channel) (int, error) {
	member := 0
...
func newBotRemoveCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runBotRemove(cmd, args[0], yes)
		},
	}
...
// runBotRemove confirms the impact, removes the bot and optionally purges the
// now-inaccessible CAR records.
func runBotRemove(cmd *cobra.Command, arg string, yes bool) error {
	ctx := cmd.Context()
	a, err := openApp(ctx, cmd, "")
	if err != nil {
		return err
