...
func bot(id int64, active bool, unavailableUntil *time.Time) store.Bot {
	return store.Bot{ID: id, Active: active, UnavailableUntil: unavailableUntil}
}

func member(botID int64, canPost, canRead, isMember bool) store.BotChannel {
	return store.BotChannel{BotID: botID, ChannelID: 1, CanPost: canPost, CanRead: canRead, Member: isMember}
}
...
func TestPickUploadBotLeastLoaded(t *testing.T) {
	now := time.Now()
	bots := []store.Bot{bot(1, true, nil), bot(2, true, nil)}
	members := []store.BotChannel{member(1, true, true, true), member(2, true, true, true)}
	lc := NewLoadCounter()
	lc.Record(1)
	lc.Record(1)
	lc.Record(2)
...
func TestPickUploadBotTieBreakByErrorsThenID(t *testing.T) {
	now := time.Now()
	bots := []store.Bot{bot(3, true, nil), bot(1, true, nil), bot(2, true, nil)}
	members := []store.BotChannel{
...
func TestPickDownloadBotPrefersFileIDOwner(t *testing.T) {
	now := time.Now()
	bots := []store.Bot{bot(1, true, nil), bot(2, true, nil)}
	members := []store.BotChannel{member(1, false, true, true), member(2, false, true, true)}
	lc := NewLoadCounter()
...
func TestPickDownloadBotLeastLoadedOwner(t *testing.T) {
	now := time.Now()
	bots := []store.Bot{bot(1, true, nil), bot(2, true, nil), bot(3, true, nil)}
	members := []store.BotChannel{
...
func TestPickDownloadBotNoneAvailable(t *testing.T) {
	now := time.Now()
	bots := []store.Bot{bot(1, false, nil)}
	members := []store.BotChannel{member(1, false, true, true)}
