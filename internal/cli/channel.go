package cli

import (
	"context"
	"errors"
	"fmt"
	"strconv"
	"time"

	"github.com/rs/zerolog/log"
	"github.com/spf13/cobra"

	"github.com/ulbwa/ipfsgram/internal/model"
	"github.com/ulbwa/ipfsgram/internal/repo"
	"github.com/ulbwa/ipfsgram/internal/tg"
)

// defaultMessageLimit mirrors the channels.message_limit schema default.
const defaultMessageLimit = 1_000_000

// newChannelCmd returns the `ipfsgram channel` command group.
func newChannelCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "channel",
		Short: "Управление каналами",
	}
	cmd.AddCommand(newChannelAddCmd(), newChannelListCmd(), newChannelRemoveCmd())
	return cmd
}

func newChannelAddCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "add <tg_id>",
		Short: "Добавить канал по Telegram ID",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			tgID, err := strconv.ParseInt(args[0], 10, 64)
			if err != nil {
				return fmt.Errorf("некорректный tg_id %q", args[0])
			}
			ctx := cmd.Context()
			e, err := openEnv(ctx, cmd)
			if err != nil {
				return err
			}
			defer e.Close()

			if _, err := e.Channels.GetByTgID(ctx, tgID); err == nil {
				return fmt.Errorf("канал %d уже добавлен", tgID)
			} else if !errors.Is(err, repo.ErrNotFound) {
				return err
			}

			bots, err := e.Bots.List(ctx)
			if err != nil {
				return err
			}
			if len(bots) == 0 {
				return errors.New("добавьте хотя бы одного бота")
			}

			tr, err := e.transport(ctx)
			if err != nil {
				return err
			}

			infos := make(map[int64]tg.ChannelInfo, len(bots))
			title := ""
			members := 0
			for _, b := range bots {
				info, perr := tr.ProbeChannel(ctx, b.Token, tgID)
				if perr != nil {
					if !errors.Is(perr, tg.ErrNoAccess) {
						log.Warn().Err(perr).Str("bot", b.Username).
							Msg("не удалось проверить доступ бота к каналу")
					}
					infos[b.ID] = tg.ChannelInfo{}
					continue
				}
				infos[b.ID] = info
				if info.Member {
					members++
					if title == "" {
						title = info.Title
					}
				}
			}
			if members == 0 {
				return fmt.Errorf("ни один бот не имеет доступа к каналу %d", tgID)
			}

			chID, err := e.Channels.Add(ctx, model.Channel{
				TgID: tgID, Title: title,
				MessageLimit: defaultMessageLimit, Active: true,
			})
			if err != nil {
				return err
			}
			for _, b := range bots {
				info := infos[b.ID]
				if err := e.Channels.UpsertBotChannel(ctx, model.BotChannel{
					BotID: b.ID, ChannelID: chID,
					CanPost: info.CanPost, CanRead: info.CanRead, CanDelete: info.CanDelete,
					Member: info.Member, VerifiedAt: time.Now(),
				}); err != nil {
					return err
				}
			}

			fmt.Fprintf(stdout, "Канал «%s» добавлен (id %d), членство ботов:\n", title, chID)
			for _, b := range bots {
				info := infos[b.ID]
				fmt.Fprintf(stdout, "  @%-24s member=%-5t post=%-5t read=%-5t delete=%t\n",
					b.Username, info.Member, info.CanPost, info.CanRead, info.CanDelete)
			}
			return nil
		},
	}
}

func newChannelListCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "list",
		Short: "Список каналов",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			e, err := openEnv(ctx, cmd)
			if err != nil {
				return err
			}
			defer e.Close()

			channels, err := e.Channels.List(ctx)
			if err != nil {
				return err
			}
			counts, err := channelBotCounts(ctx, e)
			if err != nil {
				return err
			}

			fmt.Fprintf(stdout, "%-5s %-16s %-24s %-24s %-8s %s\n",
				"ID", "TG_ID", "TITLE", "MESSAGES", "ACTIVE", "BOTS")
			for _, ch := range channels {
				pct := 0.0
				if ch.MessageLimit > 0 {
					pct = float64(ch.MessageCount) / float64(ch.MessageLimit) * 100
				}
				fmt.Fprintf(stdout, "%-5d %-16d %-24s %-24s %-8t %d\n",
					ch.ID, ch.TgID, ch.Title,
					fmt.Sprintf("%d/%d (%.1f%%)", ch.MessageCount, ch.MessageLimit, pct),
					ch.Active, counts[ch.ID])
			}
			return nil
		},
	}
}

// channelBotCounts returns, per channel, the number of member bots.
func channelBotCounts(ctx context.Context, e *env) (map[int64]int, error) {
	rows, err := e.db.QueryContext(ctx,
		`SELECT channel_id, count(*) FROM bot_channels WHERE member GROUP BY channel_id`)
	if err != nil {
		return nil, fmt.Errorf("счётчики ботов каналов: %w", err)
	}
	defer rows.Close()
	out := make(map[int64]int)
	for rows.Next() {
		var id int64
		var n int
		if err := rows.Scan(&id, &n); err != nil {
			return nil, err
		}
		out[id] = n
	}
	return out, rows.Err()
}

func newChannelRemoveCmd() *cobra.Command {
	var yes bool
	cmd := &cobra.Command{
		Use:   "remove <tg_id>",
		Short: "Удалить канал (только записи в БД)",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			tgID, err := strconv.ParseInt(args[0], 10, 64)
			if err != nil {
				return fmt.Errorf("некорректный tg_id %q", args[0])
			}
			ctx := cmd.Context()
			e, err := openEnv(ctx, cmd)
			if err != nil {
				return err
			}
			defer e.Close()

			ch, err := e.Channels.GetByTgID(ctx, tgID)
			if errors.Is(err, repo.ErrNotFound) {
				return fmt.Errorf("канал %d не найден", tgID)
			}
			if err != nil {
				return err
			}

			pins, bytes, err := e.Channels.RemoveStats(ctx, ch.ID)
			if err != nil {
				return err
			}
			prompt := fmt.Sprintf(
				"Удалить канал «%s» и %d пинов, размер %s? Записи о CAR'ах и блоках будут удалены",
				ch.Title, pins, humanBytes(bytes))
			if !yes && !Confirm(prompt) {
				fmt.Fprintln(stdout, "Отменено")
				return nil
			}

			if err := e.Channels.Remove(ctx, ch.ID); err != nil {
				return err
			}
			// Schema cascades cars/blocks/car_file_ids; pins whose blocks all
			// lived in this channel are now empty — drop them (inline query,
			// the repos expose no orphan-pin view).
			if _, err := e.db.ExecContext(ctx, `
				DELETE FROM pins p WHERE NOT EXISTS (
					SELECT 1 FROM pin_blocks pb
					JOIN blocks b ON b.cid = pb.cid
					WHERE pb.root_cid = p.root_cid
				)`); err != nil {
				return fmt.Errorf("очистка осиротевших пинов: %w", err)
			}

			fmt.Fprintf(stdout, "Канал «%s» удалён из БД. Сообщения в Telegram этой командой не удаляются.\n", ch.Title)
			return nil
		},
	}
	cmd.Flags().BoolVar(&yes, "yes", false, "не задавать вопросов, отвечать «да»")
	return cmd
}
