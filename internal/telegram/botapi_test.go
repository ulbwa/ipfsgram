// botapi_test.go — httptest-based tests for the BotAPI client: happy paths,
// error classification (flood-wait, forbidden, too big, stale file_id,
// deleted message), token redaction and the upload goroutine-leak guard.
...
func TestDownloadStaleFileID(t *testing.T) {
	...
	if !errors.Is(err, ErrBadFileID) {
		t.Fatalf("err = %v, want ErrBadFileID", err)
	}
}
...
func TestCheckMessageDeleted(t *testing.T) {
	...
	if !errors.Is(err, ErrMessageDeleted) {
		t.Fatalf("err = %v, want ErrMessageDeleted", err)
	}
}
...
func TestProbeChannel(t *testing.T) {
	...
	want := ChannelInfo{Title: "Storage", Member: true, CanPost: true, CanRead: true, CanDelete: false}
	if info != want {
