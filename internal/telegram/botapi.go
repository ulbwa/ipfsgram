// botapi.go — BotAPI, a client over the Telegram Bot API (api.telegram.org or
// a self-hosted server): token validation, channel probing, streaming upload
// via sendDocument, download via getFile, and the forward+delete CheckMessage
// trick. Also holds the UploadResult/ChannelInfo result types and the Bot API
// error classifier.
...
// BotAPI is a client over the Telegram Bot API (api.telegram.org or a
...
// NewBotAPI creates a Bot API client. An empty baseURL means the official
...
// errors. It always wraps a package sentinel via %w.
...
// case it returns ErrNoAccess, and the daemon tries other bots / other
// file_ids or MTProto.
...
// ErrMessageDeleted. This creates a short junk post in the channel; the
