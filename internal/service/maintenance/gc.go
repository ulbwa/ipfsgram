...
func (s *Service) GCCandidates(ctx context.Context) ([]domain.Car, int64, error) {
	cars, err := s.Cars.UnpinnedCars(ctx)
	if err != nil {
		return nil, 0, err
	}
	var candidates []domain.Car
	var total int64
	for _, c := range cars {
		if c.Status == domain.CarPending {
			continue
		}
...
func (s *Service) GC(ctx context.Context) (int, error) {
	deleted := 0
	err := s.Locker.WithLock(ctx, true, func(ctx context.Context) error {
		...
		botByID := make(map[int64]domain.Bot, len(bots))
		for _, b := range bots {
			botByID[b.ID] = b
		}
...
// when the row was deleted. domain.ErrMessageDeleted is tolerated; on
// domain.ErrNoAccess the car is skipped and its rows kept.
//
...
func (s *Service) deleteCar(
	ctx context.Context, car domain.Car, ch domain.Channel, botByID map[int64]domain.Bot,
) bool {
	...
	for _, m := range members {
		...
		switch {
		case err == nil, errors.Is(err, domain.ErrMessageDeleted):
			return s.dropRow(ctx, car.ID)
		case errors.As(err, &fw):
			if serr := s.Bots.SetUnavailableUntil(ctx, bot.ID, time.Now().Add(fw.RetryAfter)); serr != nil {
