// Package domain holds the IPFSgram domain layer: GORM-mapped models for every
// table in the schema, value types produced/consumed by the ports, and the
// domain errors and constants shared across ports and adapters. It has no
// dependency on infrastructure (no SQL, no Telegram, no libp2p).
package domain
...
// RawBlock is a single block produced by a BlockSource: a CID and its raw
// payload bytes. It is a value type, not a table row.
...
// PackedCar is a finished CARv1 file on disk produced by a Packer. Value type.
