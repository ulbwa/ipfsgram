...
func TestPackerSingleCarRoundTrip(t *testing.T) {
	...
	p, err := NewFactory().New(dir, 1<<20, root)
	if err != nil {
...
func TestPackerRotation(t *testing.T) {
	...
	p, err := NewFactory().New(dir, maxSize, root)
	if err != nil {
		t.Fatalf("Factory.New: %v", err)
	}
	for i := range data {
...
func TestReadAtInvariant(t *testing.T) {
	...
	for _, car := range cars {
		for _, blk := range car.Blocks {
			...
			got, err := r.ReadAt(car.Path, blk.Offset, blk.Length)
			if err != nil {
				t.Fatalf("ReadAt(%s, %d, %d): %v", car.Path, blk.Offset, blk.Length, err)
			}
			if !bytes.Equal(got, data[bi]) {
				t.Fatalf("block %d: ReadAt returned wrong bytes", bi)
			}
			bi++
		}
	}
...
func TestOversizedBlockAlone(t *testing.T) {
	...
	got, err := NewReader().ReadAt(bigCar.Path, bigCar.Blocks[0].Offset, bigCar.Blocks[0].Length)
	if err != nil {
...
func TestOversizedSingleBlockOnly(t *testing.T) {
	...
	p, err := NewFactory().New(dir, maxSize, c)
	if err != nil {
		t.Fatalf("Factory.New: %v", err)
	}
