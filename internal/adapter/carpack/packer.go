// Package carpack is the CAR adapter: it writes IPFS blocks into size-capped
// CARv1 archives and records, for every block, the absolute byte offset of its
// payload inside the archive. This lets a reader serve a block later with a
// single ReadAt(offset, length) call, without parsing the CAR on the read path.
// It implements the port.Packer, port.PackerFactory, and port.BlockReader
// contracts over domain value types.
package carpack
...
// Compile-time assertions that the adapter satisfies its ports.
var (
	_ port.Packer        = (*Packer)(nil)
	_ port.PackerFactory = (*Factory)(nil)
)

// Packer writes blocks into CARv1 files, rotating to a new file when the
// configured size cap would be exceeded. Every produced file carries the same
// root in its header. The limit is best effort: a single block whose section
// does not fit into maxSize is written alone into its own CAR.
//
...
type Packer struct {
	...
	current  domain.PackedCar
	finished []domain.PackedCar
	done     bool
}

// newPacker creates a packer that writes CAR files named car-000001.car,
// car-000002.car, ... inside dir. Each file's CARv1 header declares root as the
// single root CID. Files are only created once the first block is added.
func newPacker(dir string, maxSize int64, root cid.Cid) (*Packer, error) {
	if maxSize <= 0 {
...
func (p *Packer) Add(c cid.Cid, data []byte) error {
	if p.done {
	...
	p.size += sectionSize
	p.current.Blocks = append(p.current.Blocks, domain.PackedBlock{
		CID:    c,
...
func (p *Packer) Finish() ([]domain.PackedCar, error) {
	if p.done {
...
func (p *Packer) openNext() error {
	...
	p.current = domain.PackedCar{Path: path}
	return nil
}
