...
// ordering, while this adapter keeps the running total and, after every Add,
...
// adapter's own mutex, onEvict touches no adapter mutex — it only deletes the
...
func NewLRU(dir string, maxBytes int64) (port.Cache, error) {
	if maxBytes <= 0 {
		return nil, fmt.Errorf("diskcache: maxBytes must be positive, got %d", maxBytes)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("diskcache: new lru: %w", err)
	}
	c.lru = l
...
// its size from the running total. It must not touch any adapter mutex (see the
