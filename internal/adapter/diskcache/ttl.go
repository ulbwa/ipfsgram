...
// entries; this adapter's callback deletes the file.
//
...
// time T is removed at T+ttl regardless of how often it is read. This differs
// from the old hand-written ttlCache (where Get refreshed the deadline); it is
// the natural, documented behavior of the ready-made library and acceptable for
// a re-downloadable CAR cache.
//
...
// call back into the lru and must not take any adapter mutex that is ever held
...
func NewTTL(dir string, ttl time.Duration) (port.Cache, error) {
	if ttl <= 0 {
		return nil, fmt.Errorf("diskcache: ttl must be positive, got %v", ttl)
	}
...
// adapter mutex held across an lru call.
