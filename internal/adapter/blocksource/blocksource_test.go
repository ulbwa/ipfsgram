...
func TestFileSourceSmall(t *testing.T) {
	data := []byte("hello ipfsgram")
	root, blocks, err := NewFile(writeTempFile(t, data)).Load(context.Background())
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if len(blocks) != 1 {
...
func TestFileSourceMultiChunk(t *testing.T) {
	...
	root2, _, err := NewFile(writeTempFile(t, data)).Load(context.Background())
	if err != nil {
		t.Fatal(err)
	}
