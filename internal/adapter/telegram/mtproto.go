...
// MTProto is a transport over gotd/td authorized with a bot token. It removes
...
// NewMTProto creates an MTProto transport.
...
func classifyMTProtoError(err error) error {
	...
	if errors.As(err, &fw) || errors.Is(err, domain.ErrMessageDeleted) ||
		errors.Is(err, domain.ErrNoAccess) || errors.Is(err, domain.ErrTooLarge) || errors.Is(err, domain.ErrBadFileID) {
		return err
	}
	if d, ok := tgerr.AsFloodWait(err); ok {
		return fmt.Errorf("tg: mtproto: %v: %w", err, &domain.FloodWaitError{RetryAfter: d})
	}
	if tgerr.Is(err, "MESSAGE_ID_INVALID", "MESSAGE_IDS_EMPTY") {
		return fmt.Errorf("tg: mtproto: %v: %w", err, domain.ErrMessageDeleted)
	}
	if tgerr.Is(err,
		...
		return fmt.Errorf("tg: mtproto: %v: %w", err, domain.ErrNoAccess)
	}
	return err
}
...
func resolveChannel(ctx context.Context, api *tdtg.Client, channelTgID int64) (*tdtg.InputChannel, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("tg: mtproto: resolve channel %d: %v: %w", channelTgID, err, domain.ErrNoAccess)
	}
	...
	return nil, fmt.Errorf("tg: mtproto: channel %d not returned by getChannels: %w", channelTgID, domain.ErrNoAccess)
}
...
func channelDocument(ctx context.Context, api *tdtg.Client, ch *tdtg.InputChannel, messageID int64) (*tdtg.Document, error) {
	...
	if !ok {
		return nil, fmt.Errorf("tg: mtproto: unexpected getMessages result %T: %w", res, domain.ErrNoAccess)
	}
	if len(msgs.Messages) == 0 {
		return nil, fmt.Errorf("tg: mtproto: message %d not found: %w", messageID, domain.ErrMessageDeleted)
	}
	msg, ok := msgs.Messages[0].(*tdtg.Message)
	if !ok {
		// messageEmpty -> the message was deleted.
		return nil, fmt.Errorf("tg: mtproto: message %d is empty: %w", messageID, domain.ErrMessageDeleted)
	}
	media, ok := msg.Media.(*tdtg.MessageMediaDocument)
	if !ok {
		return nil, fmt.Errorf("tg: mtproto: message %d has no document media: %w", messageID, domain.ErrMessageDeleted)
	}
	doc, ok := media.Document.AsNotEmpty()
	if !ok {
		return nil, fmt.Errorf("tg: mtproto: message %d document is empty: %w", messageID, domain.ErrMessageDeleted)
	}
	return doc, nil
}
...
// Upload via MTProto is not implemented: the factory always routes uploads
// through Bot API (sendDocument yields a file_id, which is needed for
// subsequent downloads by other bots).
func (m *MTProto) Upload(context.Context, string, int64, string, int64, io.Reader) (port.UploadResult, error) {
	return port.UploadResult{}, errors.New("tg: mtproto: upload not supported, use bot api transport")
}
