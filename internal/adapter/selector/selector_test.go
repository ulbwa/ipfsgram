...
func bot(id int64, active bool, unavailableUntil *time.Time) domain.Bot {
	return domain.Bot{ID: id, Active: active, UnavailableUntil: unavailableUntil}
}

func member(botID int64, canPost, canRead, isMember bool) domain.BotChannel {
	return domain.BotChannel{BotID: botID, ChannelID: 1, CanPost: canPost, CanRead: canRead, Member: isMember}
}
...
func TestPickUploadBotFilters(t *testing.T) {
	...
	got, err := New().PickUploadBot(now, bots, members)
	if err != nil {
...
func TestPickUploadBotLeastLoaded(t *testing.T) {
	now := time.Now()
	bots := []domain.Bot{bot(1, true, nil), bot(2, true, nil)}
	members := []domain.BotChannel{member(1, true, true, true), member(2, true, true, true)}
	s := New()
	s.Record(1)
	s.Record(1)
	s.Record(2)
	got, err := s.PickUploadBot(now, bots, members)
	if err != nil {
		t.Fatalf("PickUploadBot: %v", err)
	}
	if got.ID != 2 {
...
func TestPickUploadBotTieBreakByErrorsThenID(t *testing.T) {
	now := time.Now()
	bots := []domain.Bot{bot(3, true, nil), bot(1, true, nil), bot(2, true, nil)}
	members := []domain.BotChannel{
	...
	s := New()
	s.RecordError(1) // same load, more errors -> lose tie-break
...
func TestPickUploadBotNoneAvailable(t *testing.T) {
	...
	members := []domain.BotChannel{member(1, true, true, true), member(2, true, true, true)}
	_, err := New().PickUploadBot(now, bots, members)
...
func TestPickDownloadBotPrefersFileIDOwner(t *testing.T) {
	now := time.Now()
	bots := []domain.Bot{bot(1, true, nil), bot(2, true, nil)}
	members := []domain.BotChannel{member(1, false, true, true), member(2, false, true, true)}
	s := New()
	s.Record(1) // owner more loaded than non-owner
	s.Record(1)
	got, fileID, err := s.PickDownloadBot(now, bots, members, map[int64]string{1: "file-1"})
	if err != nil {
...
func TestPickDownloadBotLeastLoadedOwner(t *testing.T) {
	now := time.Now()
	bots := []domain.Bot{bot(1, true, nil), bot(2, true, nil), bot(3, true, nil)}
	members := []domain.BotChannel{
	...
	s := New()
	s.Record(1)
	got, fileID, err := s.PickDownloadBot(now, bots, members, fileIDs)
	if err != nil {
		t.Fatalf("PickDownloadBot: %v", err)
	}
...
func TestPickDownloadBotOwnerFloodWaitFallsBack(t *testing.T) {
	...
	members := []domain.BotChannel{member(1, false, true, true), member(2, false, true, true)}
	got, fileID, err := New().PickDownloadBot(now, bots, members, map[int64]string{1: "file-1"})
	if err != nil {
		t.Fatalf("PickDownloadBot: %v", err)
	}
	if got.ID != 2 || fileID != "" {
...
func TestPickDownloadBotRequiresCanRead(t *testing.T) {
	...
	got, _, err := New().PickDownloadBot(now, bots, members, nil)
	if err != nil {
		t.Fatalf("PickDownloadBot: %v", err)
	}
	if got.ID != 2 {
...
func TestPickDownloadBotNoneAvailable(t *testing.T) {
	now := time.Now()
	bots := []domain.Bot{bot(1, false, nil)}
	members := []domain.BotChannel{member(1, false, true, true)}
	_, _, err := New().PickDownloadBot(now, bots, members, map[int64]string{1: "file-1"})
	if !errors.Is(err, port.ErrNoBotAvailable) {
		t.Errorf("err = %v, want ErrNoBotAvailable", err)
	}
}
...
func TestPickChannelFillFirst(t *testing.T) {
	...
	ch, warn, err := New().PickChannel(channels, 0.95)
	if err != nil {
...
func TestPickChannelTieBreakByID(t *testing.T) {
	...
	ch, _, err := New().PickChannel(channels, 0.95)
	if err != nil {
		t.Fatalf("PickChannel: %v", err)
	}
	if ch.ID != 1 {
...
func TestPickChannelNoSpace(t *testing.T) {
	...
	if _, _, err := New().PickChannel(full, 0.95); !errors.Is(err, port.ErrNoChannelSpace) {
		t.Errorf("err = %v, want ErrNoChannelSpace", err)
	}
	if _, _, err := New().PickChannel(nil, 0.95); !errors.Is(err, port.ErrNoChannelSpace) {
