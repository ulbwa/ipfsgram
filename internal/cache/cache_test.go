// cache_test.go — LRU and TTL disk cache tests: roundtrip, eviction,
// recency, oversized entries, concurrency, restart adoption, idempotent
// close.
...
func TestLRUCloseIdempotent(t *testing.T) {
	c, err := NewLRU(t.TempDir(), 1<<20)
	if err != nil {
		t.Fatalf("NewLRU: %v", err)
	}
...
// TestTTLExpiry verifies an entry's file is removed by the library reaper once
// the ttl elapses. It polls for file disappearance rather than calling Get so
// the test does not depend on any access-refresh behavior.
...
func TestTTLPutGetRoundtrip(t *testing.T) {
	dir := t.TempDir()
	...
	content := []byte("hello ttl")
	path, err := c.Put(3, writeTempFile(t, content), int64(len(content)))
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
	got, ok := c.Get(3)
...
func TestTTLCloseIdempotent(t *testing.T) {
	c, err := NewTTL(t.TempDir(), time.Minute)
	if err != nil {
		t.Fatalf("NewTTL: %v", err)
	}
	if err := c.Close(); err != nil {
