// dedup.go — the pure dedup decision: which already-stored blocks can be
// skipped and which cars must be cleaned up before re-upload.

package publish

import "github.com/ulbwa/ipfsgram/internal/store"
...
type dedupPlan struct {
	...
	// be marked with.
	SetStatus map[int64]store.CarStatus
}
...
func planDedup(
	...
	for carID, st := range statuses {
		...
		case checkDeleted:
			...
			deleted[carID] = true
		case checkNoAccess:
			plan.SetStatus[carID] = store.CarNoBotAccess
		case checkTooLarge:
			plan.SetStatus[carID] = store.CarTooLarge
		}
	}
