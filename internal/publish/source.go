// source.go — block sources for the publish pipeline: LoadFile imports a local
// file as a UnixFS DAG, FetchDAG pulls a DAG from the public IPFS network.
...
// RawBlock is a block held in memory: its CID and raw payload.
type RawBlock struct {
	CID  cid.Cid
	Data []byte
}

// LoadFile imports the file at path into a balanced UnixFS DAG (256 KiB fixed
...
// file size.
func LoadFile(ctx context.Context, path string) (cid.Cid, []RawBlock, error) {
	f, err := os.Open(path)
	if err != nil {
...
// FetchDAG spins up a temporary lightweight IPFS client node (libp2p host +
...
func FetchDAG(ctx context.Context, root cid.Cid) (cid.Cid, []RawBlock, error) {
...
func collectDAG(ctx context.Context, ng ipld.NodeGetter, root cid.Cid) ([]RawBlock, error) {
	var blocks []RawBlock
	...
	for len(stack) > 0 {
		...
		blocks = append(blocks, RawBlock{CID: c, Data: nd.RawData()})
